...
#cmt_local_tagfile = $(DatabaseSvc_tag).make
cmt_local_tagfile = $(bin)$(DatabaseSvc_tag).make
...
cmt_local_tagfile_install_more_includes = $(bin)$(DatabaseSvc_tag)_install_more_includes.make
...
cmt_local_tagfile_install_more_includes = $(bin)$(DatabaseSvc_tag).make
...
DatabaseSvc_extratags = -tag_add=target_DatabaseSvc
...
not_DatabaseSvccompile_dependencies = { n=0; for p in $?; do m=0; for d in $(DatabaseSvccompile_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
DatabaseSvcdirs :
	@if test ! -d $(bin)DatabaseSvc; then $(mkdir) -p $(bin)DatabaseSvc; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)DatabaseSvc
else
DatabaseSvcdirs : ;
endif
...
ifndef QUICK
$(cmt_local_DatabaseSvc_makefile) : $(DatabaseSvccompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building DatabaseSvc.make"; \
	  $(cmtexe) -tag=$(tags) $(DatabaseSvc_extratags) build constituent_config -out=$(cmt_local_DatabaseSvc_makefile) DatabaseSvc
else
$(cmt_local_DatabaseSvc_makefile) : $(DatabaseSvccompile_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_DatabaseSvc) ] || \
	  [ ! -f $(cmt_final_setup_DatabaseSvc) ] || \
	  $(not_DatabaseSvccompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building DatabaseSvc.make"; \
	  $(cmtexe) -tag=$(tags) $(DatabaseSvc_extratags) build constituent_config -out=$(cmt_local_DatabaseSvc_makefile) DatabaseSvc; \
	  fi
endif
...
$(cmt_local_DatabaseSvc_makefile) : $(DatabaseSvccompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building DatabaseSvc.make"; \
	  $(cmtexe) -f=$(bin)DatabaseSvc.in -tag=$(tags) $(DatabaseSvc_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_DatabaseSvc_makefile) DatabaseSvc
else
$(cmt_local_DatabaseSvc_makefile) : $(DatabaseSvccompile_dependencies) $(cmt_build_library_linksstamp) $(bin)DatabaseSvc.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_DatabaseSvc) ] || \
	  [ ! -f $(cmt_final_setup_DatabaseSvc) ] || \
	  $(not_DatabaseSvccompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(DatabaseSvc_extratags) build constituent_makefile -out=$(cmt_local_DatabaseSvc_makefile) DatabaseSvc

DatabaseSvc :: DatabaseSvccompile DatabaseSvcinstall ;

ifdef cmt_DatabaseSvc_has_prototypes

DatabaseSvcprototype : $(DatabaseSvcprototype_dependencies) $(cmt_local_DatabaseSvc_makefile) dirs DatabaseSvcdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_DatabaseSvc_makefile); then \
	  $(MAKE) -f $(cmt_local_DatabaseSvc_makefile) $@; \
	  fi
#	@$(MAKE) -f $(cmt_local_DatabaseSvc_makefile) $@
	$(echo) "(constituents.make) $@ done"

DatabaseSvccompile : DatabaseSvcprototype

endif

DatabaseSvccompile : $(DatabaseSvccompile_dependencies) $(cmt_local_DatabaseSvc_makefile) dirs DatabaseSvcdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_DatabaseSvc_makefile); then \
	  $(MAKE) -f $(cmt_local_DatabaseSvc_makefile) $@; \
	  fi
...
DatabaseSvcclean :: $(DatabaseSvcclean_dependencies) ##$(cmt_local_DatabaseSvc_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_DatabaseSvc_makefile); then \
	  $(MAKE) -f $(cmt_local_DatabaseSvc_makefile) $@; \
	fi
	$(echo) "(constituents.make) $@ done"
#	@-$(MAKE) -f $(cmt_local_DatabaseSvc_makefile) DatabaseSvcclean

##	  /bin/rm -f $(cmt_local_DatabaseSvc_makefile) $(bin)DatabaseSvc_dependencies.make

install :: DatabaseSvcinstall ;

DatabaseSvcinstall :: DatabaseSvccompile $(DatabaseSvc_dependencies) $(cmt_local_DatabaseSvc_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_DatabaseSvc_makefile); then \
	  $(MAKE) -f $(cmt_local_DatabaseSvc_makefile) $@; \
	  fi
...
$(foreach d,$(DatabaseSvc_dependencies),$(eval $(d)uninstall_dependencies += DatabaseSvcuninstall))

DatabaseSvcuninstall : $(DatabaseSvcuninstall_dependencies) ##$(cmt_local_DatabaseSvc_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_DatabaseSvc_makefile); then \
	  $(MAKE) -f $(cmt_local_DatabaseSvc_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_DatabaseSvc_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: DatabaseSvcuninstall ;

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(constituents.make) Starting $@ DatabaseSvc"
	$(echo) Using default action for $@
	$(echo) "(constituents.make) $@ DatabaseSvc done"
endif
...
cmt_local_tagfile_make = $(bin)$(DatabaseSvc_tag)_make.make
...
cmt_local_tagfile_make = $(bin)$(DatabaseSvc_tag).make
