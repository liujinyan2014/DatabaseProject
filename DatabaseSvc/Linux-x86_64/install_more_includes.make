...
#cmt_local_tagfile_install_more_includes = $(DatabaseSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(DatabaseSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)DatabaseSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)DatabaseSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "DatabaseSvc" = ""; then \
	    if test -d ../DatabaseSvc; then \
	      echo "Installing files from more ../DatabaseSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/DatabaseSvc; then mkdir -p $(install_include_dir)/DatabaseSvc; fi;\
	        $(install_command) ../DatabaseSvc $(install_include_dir);\
	    else \
	      echo "No more DatabaseSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/DatabaseSvc"; \
	  $(uninstall_command) "$(install_include_dir)/DatabaseSvc/*" ; \
	  if test -f $(install_include_dir)/DatabaseSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/DatabaseSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/DatabaseSvc.cmtref ; \
	  fi; \
	fi
